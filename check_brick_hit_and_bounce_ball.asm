...
check_case_up_left:
	...
	jp z,down_right_main_compare		;9dd3	ca 6b 9f
	bit 7,(iy+BALL_TABLE_IDX_X_SPEED)	;9dd6	fd cb 03 7e
	jp z,down_right_main_compare		;9dda	ca 6b 9f
	...
	srl a		;9de6	cb 3f
	cp 12		;9de8	fe 0c
	jp nc,down_right_main_compare	;9dea	d2 6b 9f
	ld (CURR_BRICK_Y),a		        ;9ded	32 8a e5
	...
	srl a		;9e2a	cb 3f
	cp 11		;9e2c	fe 0b
	jp nc,down_right_main_compare		;9e2e	d2 6b 9f
	ld (PREV_BRICK_X),a		            ;9e31	32 8d e5
	...
	cp 11		                        ;9e45	fe 0b
	jp nz,up_left_main_compare		    ;9e47	c2 58 9e
	ld a,(PREV_BRICK_Y)		            ;9e4a	3a 8c e5
	cp 12		                        ;9e4d	fe 0c
	jp nz,up_left_main_compare		    ;9e4f	c2 58 9e
...
up_left_main_compare:
	...
	cp c			                ;9e67	b9
	jp z,up_left_same_row		    ;9e68	ca 73 9e
	dec c			                ;9e6b	0d
	cp c			                ;9e6c	b9
...
down_right_main_compare:
	...
	ld (CURR_BRICK_Y),a		        ;9f89	32 8a e5
	...
	ld (PREV_BRICK_X),a		    ;9fc8	32 8d e5
...
la00ah:
	...
	cp c			            ;a011	b9
	jp z,la030h		            ;a012	ca 30 a0
	inc c			            ;a015	0c
	cp c			            ;a016	b9
	jp z,la04bh		            ;a017	ca 4b a0
...
la030h:
	...
	ld (BRICK_ROW),a		;a033	32 aa e2
	ld a,(CURR_BRICK_X)		;a036	3a 8b e5
	ld (BRICK_COL),a		;a039	32 ab e2
...
la04bh:
	...
	ld (BRICK_ROW),a		        ;a04e	32 aa e2
	ld a,(CURR_BRICK_X)		        ;a051	3a 8b e5
	ld (BRICK_COL),a		        ;a054	32 ab e2
...
la069h:
	...
	ld (BRICK_ROW),a		;a06c	32 aa e2
	ld a,(PREV_BRICK_X)		;a06f	3a 8d e5
	ld (BRICK_COL),a		;a072	32 ab e2
...
la087h:
	...
	ld (BRICK_ROW),a		;a08a	32 aa e2
	ld a,(PREV_BRICK_X)		;a08d	3a 8d e5
	ld (BRICK_COL),a		;a090	32 ab e2
	call RESOLVE_CORNER_COLLISION		;a093	cd 70 a6
	...
	ld (ix+SPR_PARAMS_IDX_Y),a		;a0a2	dd 77 00
	ld a,(BRICK_HIT_X_PIXEL)		;a0a5	3a 3d e5
	ld (ix+SPR_PARAMS_IDX_X),a		;a0a8	dd 77 01
...
la0cch:
	...
	ld (BRICK_ROW),a		;a0cf	32 aa e2
	ld a,(CURR_BRICK_X)		;a0d2	3a 8b e5
	ld (BRICK_COL),a		;a0d5	32 ab e2
...
check_case_down_left:
	...
	ld (CURR_BRICK_Y),a		;a120	32 8a e5
	...
	ld (PREV_BRICK_X),a		        ;a15f	32 8d e5
...
la1c7h:
	...
	ld (BRICK_ROW),a		;a1ca	32 aa e2
	ld a,(CURR_BRICK_X)		;a1cd	3a 8b e5
	ld (BRICK_COL),a		;a1d0	32 ab e2
...
la1e2h:
	...
	ld (BRICK_ROW),a		;a1e5	32 aa e2
	ld a,(CURR_BRICK_X)		;a1e8	3a 8b e5
	ld (BRICK_COL),a		;a1eb	32 ab e2
...
la200h:
	...
	ld (BRICK_ROW),a		;a203	32 aa e2
	ld a,(PREV_BRICK_X)		;a206	3a 8d e5
	ld (BRICK_COL),a		;a209	32 ab e2
...
la21eh:
	...
	ld (BRICK_ROW),a		;a221	32 aa e2
	ld a,(PREV_BRICK_X)		;a224	3a 8d e5
	ld (BRICK_COL),a		;a227	32 ab e2

	call RESOLVE_CORNER_COLLISION	;a22a	cd 70 a6
	jp nc,la263h		            ;a22d	d2 63 a2

	call BRICK_EXISTS_AT_ROWCOL		;a230	cd a8 ad
	jp nc,la24bh		            ;a233	d2 4b a2

	ld a,(BRICK_HIT_Y_PIXEL)		;a236	3a 3c e5
	ld (ix+SPR_PARAMS_IDX_Y),a		;a239	dd 77 00
	ld a,(BRICK_HIT_X_PIXEL)		;a23c	3a 3d e5
	ld (ix+SPR_PARAMS_IDX_X),a		;a23f	dd 77 01
...
la263h:
	...
	ld (BRICK_ROW),a		;a266	32 aa e2
	ld a,(CURR_BRICK_X)		;a269	3a 8b e5
	ld (BRICK_COL),a		;a26c	32 ab e2
...
la2eeh:
	...
	ld (ix+SPR_PARAMS_IDX_Y),a		;a309	dd 77 00
	ld a,(BRICK_HIT_X_PIXEL)		;a30c	3a 3d e5
	ld (ix+SPR_PARAMS_IDX_X),a		;a30f	dd 77 01
...
la38eh:
	...
	ld (BRICK_ROW),a		;a391	32 aa e2
	ld a,(CURR_BRICK_X)		;a394	3a 8b e5
	ld (BRICK_COL),a		;a397	32 ab e2
...
la3afh:
	...
	ld (BRICK_ROW),a		;a3b2	32 aa e2
	ld a,(CURR_BRICK_X)		;a3b5	3a 8b e5
	ld (BRICK_COL),a		;a3b8	32 ab e2
...
COMPUTE_PRECISE_HIT_POINT:
	ld hl,TICKS_TO_HIT		;a3d1	21 41 e5
	ld (hl), 0		        ;a3d4	36 00
	ld de,BALL_X_SLOPE		;a3d6	11 42 e5
...
la3f4h:
	add a,b			                    ;a3f4	80
	ld (COMPUTED_HIT_X),a		        ;a3f5	32 c7 e2
	add a, 15		                    ;a3f8	c6 0f
	ld (COMPUTED_HIT_X_NEG),a	        ;a3fa	32 c6 e2
...
la407h:
	...
	ld l,a			                    ;a40a	6f
	ld h,0		                        ;a40b	26 00
	ld de,TBL_SPEED_FROM_SKEWNESS		;a40d	11 6c a8
	add hl,de			                ;a410	19
	ld a,(hl)			                ;a411	7e
	...
	neg		                            ;a419	ed 44
la41bh:
	ld (BALL_X_SLOPE),a		            ;a41b	32 42 e5
	inc hl			                    ;a41e	23
	ld a,(hl)			                ;a41f	7e
	...
	neg		                            ;a427	ed 44
la429h:
	ld (BALL_Y_SLOPE),a	;a429	32 43 e5
	ld a,(BALL_X_SLOPE)	;a42c	3a 42 e5
	ld b,a			    ;a42f	47
...
la441h:
	push af			        ;a441	f5
	ld a,(TICKS_TO_HIT)		;a442	3a 41 e5
	inc a			        ;a445	3c
	ld (TICKS_TO_HIT),a		;a446	32 413c
	pop af			;a449	f1
	add a,b			;a44a	80
	cp (hl)			;a44b	be  Compare with COMPUTED_HIT_X
	jp nc,la463h	;a44c	d2 63 a4
	jp la441h		;a44f	c3 41 a4
la452h:
	push af			        ;a452	f5
	ld a,(TICKS_TO_HIT)		;a453	3a 41 e5
	inc a			        ;a456	3c
	ld (TICKS_TO_HIT),a		;a457	32 41
	pop af			;a45a	f1
	add a,b			;a45b	80
	cp (hl)			;a45c	be
	jp c,la464h		;a45d	da 64 a4
	jp la452h		;a460	c3 52 a4
la463h:
	dec (hl)			;a463	35
...
la4cch:
	ld (BRICK_HIT_X_PIXEL),a		;a4cc	32 3d e5
	ld hl,TICKS_TO_HIT		        ;a4cf	21 41 e5
	ld (hl), 0		                ;a4d2	36 00
	ld de,BALL_X_SLOPE		        ;a4d4	11 42 e5
...
la4f0h:
	add a,b			        ;a4f0	80
	ld (HIY_Y_EDGE_A),a		;a4f1	32 c4 e2
	add a, 7		        ;a4f4	c6 07
	ld (HIY_Y_EDGE_B),a		;a4f6	32 c5 e2
...
la503h:
	...
	ld l,a			;a506	6f
	ld h,0		    ;a507	26 00
	ld de,TBL_SPEED_FROM_SKEWNESS		;a509	11 6c a8
	add hl,de			                ;a50c	19
	ld a,(hl)			                ;a50d	7e
	...
	neg		                            ;a515	ed 44
la517h:
	ld (BALL_X_SLOPE),a		            ;a517	32 42 e5
	inc hl			                    ;a51a	23
	ld a,(hl)			                ;a51b	7e
	...
	neg		                            ;a523	ed 44
la525h:
	ld (BALL_Y_SLOPE),a		;a525	32 43 e5
	ld a,(BALL_Y_SLOPE)		;a528	3a 43 e5
	ld b,a			        ;a52b	47
	ld a,(PREV_Y_PX)		;a52c	3a 86 e5
la52fh:
	ld hl,HIY_Y_EDGE_B		            ;a52f	21 c5 e2
	bit 7,(iy+BALL_TABLE_IDX_Y_SPEED)	;a532	fd cb 02
	jp nz,la54eh		                ;a536	c2 4e a5
	ld hl,HIY_Y_EDGE_A		            ;a539	21 c4 e2
	inc (hl)			                ;a53c	34
la53dh:
	push af			        ;a53d	f5
	ld a,(TICKS_TO_HIT)		;a53e	3a 41 e5
	inc a			        ;a541	3c
	ld (TICKS_TO_HIT),a		;a542	32 413c
	pop af			        ;a545	f1
	add a,b			        ;a546	80
	cp (hl)			        ;a547	be  Compare with HIY_Y_EDGE_A
	jp nc,la55fh		    ;a548	d2 5f a5
	jp la53dh		        ;a54b	c3 3d a5
la54eh:
	push af			        ;a54e	f5
	ld a,(TICKS_TO_HIT)		;a54f	3a 41 e5
	inc a			        ;a552	3c
	ld (TICKS_TO_HIT),a		;a553	32 413c
	pop af			;a556	f1
	add a,b			;a557	80
	cp (hl)			;a558	be
	jp c,la560h		;a559	da 60 a5
	jp la54eh		;a55c	c3 4e a5
la55fh:
	dec (hl)			;a55f	35
la560h:
	ld a,(TICKS_TO_HIT)		;a560	3a 41 e5
	ld b,a			        ;a563	47
	ld a,(BALL_X_SLOPE)		;a564	3a 42 e5
	ld c,a			        ;a567	4f
	neg		                ;a568	ed 44
la56ah:
	add a,c			;a56a	81
	...
	add a,b			        ;a571	80
	ld (BRICK_HIT_Y_PIXEL),a		;a572	32 3c e5
	ld b,a			                ;a575	47
	ld a, 188		                ;a576	3e bc
...
COMPUTE_WALL_ADJACENT_HIT_POINT:
	ld hl,TICKS_TO_HIT		;a591	21 41 e5
	ld (hl), 0		        ;a594	36 00
	ld de,BALL_X_SLOPE		;a596	11 42 e5
...
la5b2h:
	add a,b			        ;a5b2	80
	ld (HIY_Y_EDGE_A),a		;a5b3	32 c4 e2

	add a, 7		        ;a5b6	c6 07
	ld (HIY_Y_EDGE_B),a		;a5b8	32 c5 e2
...
la5c5h:
	...
	ld l,a		;a5c8	6f
	ld h, 0		;a5c9	26 00
	ld de,TBL_SPEED_FROM_SKEWNESS		;a5cb	11 6c a8
	add hl,de			                ;a5ce	19
	ld a,(hl)			                ;a5cf	7e
	...
	neg		                            ;a5d7	ed 44
la5d9h:
	ld (BALL_X_SLOPE),a		;a5d9	32 42 e5
	inc hl			        ;a5dc	23
	ld a,(hl)			    ;a5dd	7e
	...
	neg		                                ;a5e5	ed 44
la5e7h:
	ld (BALL_Y_SLOPE),a		;a5e7	32 43 e5
	ld a,(BALL_Y_SLOPE)		;a5ea	3a 43 e5
	ld b,a			        ;a5ed	47
...
la5ffh:
	push af			        ;a5ff	f5
	ld a,(TICKS_TO_HIT)		;a600	3a 41 e5
	inc a			        ;a603	3c
	ld (TICKS_TO_HIT),a		;a604	32 413c
	pop af			;a607	f1
	add a,b			;a608	80
	cp (hl)			;a609	be
...
la610h:
	push af			        ;a610	f5
	ld a,(TICKS_TO_HIT)		;a611	3a 41 e5
	inc a			        ;a614	3c
	ld (TICKS_TO_HIT),a		;a615	32 413c
	pop af			        ;a618	f1
	add a,b			        ;a619	80
	cp (hl)			        ;a61a	be
...
la621h:
	dec (hl)			    ;a621	35
la622h:
	ld a,(TICKS_TO_HIT)		;a622	3a 41 e5
	ld b,a			        ;a625	47
	ld a,(BALL_X_SLOPE)		;a626	3a 42 e5
	ld c,a			        ;a629	4f
	neg		                ;a62a	ed 44
la62ch:
	add a,c			 ;a62c	81
	...
	jp z,la64eh		 ;a63b	ca 4e a6
	ld a, 15		 ;a63e	3e 0f
	cp b			 ;a640	b8
	jp c,la64ah		 ;a641	da 4a a6
	or a			 ;a644	b7
	ld b,a			 ;a645	47
	inc b			 ;a646	04
...
RESOLVE_CORNER_COLLISION:
    ...
    ; Clear 2 variables
	ld hl,TICKS_TO_HIT		;a670	21 41 e5
	ld (hl), 0		        ;a673	36 00
	ld de,BALL_X_SLOPE		;a675	11 42 e5
...
la691h:
	add a,b			        ;a691	80
	ld (HIY_Y_EDGE_A),a		;a692	32 c4 e2
	add a, 7		        ;a695	c6 07
	ld (HIY_Y_EDGE_B),a		;a697	32 c5 e2
...
la6a4h:
	...
	ld l,a		;a6a7	6f
	ld h, 0		;a6a8	26 00
	ld de,TBL_SPEED_FROM_SKEWNESS		;a6aa	11 6c a8
	add hl,de	;a6ad	19
	...
	neg		                            ;a6b6	ed 44
la6b8h:
	ld (BALL_X_SLOPE),a		;a6b8	32 42 e5
	inc hl			        ;a6bb	23
	ld a,(hl)			    ;a6bc	7e
	...
	neg		                                ;a6c4	ed 44
la6c6h:
	ld (BALL_Y_SLOPE),a		;a6c6	32 43 e5
...
la7cah:
	...
	ld (BRICK_HIT_Y_PIXEL),a	;a7ce	32 3c e5
	ld a,b			            ;a7d1	78
	ld (BRICK_HIT_X_PIXEL),a	;a7d2	32 3d e5
	pop af			            ;a7d5	f1
	ret			                ;a7d6	c9
la7d7h:
...
la803h:
	...
	ld (BRICK_HIT_Y_PIXEL),a		;a807	32 3c e5
	ld a,b			                ;a80a	78
	ld (BRICK_HIT_X_PIXEL),a		;a80b	32 3d e5
	pop af			                ;a80e	f1
	ret			                    ;a80f	c9
...
la891h:
	push af			        ;a891	f5
	ld a,(TICKS_TO_HIT)		;a892	3a 41 e5
	inc a			        ;a895	3c
	ld (TICKS_TO_HIT),a		;a896	32 413c
	pop af			        ;a899	f1
	add a,b			        ;a89a	80
	cp (hl)			        ;a89b	be
...
la8a2h:
	push af			        ;a8a2	f5
	ld a,(TICKS_TO_HIT)		;a8a3	3a 41 e5
	inc a			        ;a8a6	3c
	ld (TICKS_TO_HIT),a		;a8a7	32 413c
	pop af			;a8aa	f1
	add a,b			;a8ab	80
	cp (hl)			;a8ac	be
...
la8b3h:
	dec (hl)			;a8b3	35
la8b4h:
	ld a,(TICKS_TO_HIT)		;a8b4	3a 41 e5
	ld b,a			        ;a8b7	47
	ld a,(BALL_X_SLOPE)		;a8b8	3a 42 e5
	ld c,a			        ;a8bb	4f
	neg		                ;a8bc	ed 44
la8beh:
	add a,c			;a8be	81
...
la8e6h:
	...
	ld b,a			;a8ec	47
la8edh:
	ld (ix+SPR_PARAMS_IDX_X),b		;a8ed	dd 70 01
...
la8fdh:
	ld (ix+SPR_PARAMS_IDX_Y),a		;a8fd	dd 77 00
	ret			                    ;a900	c9
...
la94bh:
	; Set BALL_X_SLOPE
    ld (BALL_X_SLOPE),a		;a94b	32 42 e5
...
la971h:
	push af			        ;a971	f5
	ld a,(TICKS_TO_HIT)		;a972	3a 41 e5
	inc a			        ;a975	3c
	ld (TICKS_TO_HIT),a		;a976	32 413c
	pop af			        ;a979	f1
	add a,b			        ;a97a	80
	cp (hl)			        ;a97b	be
	jp nc,la993h		    ;a97c	d2 93 a9
	jp la971h		    ;a97f	c3 71 a9
la982h:
	push af			        ;a982	f5
	ld a,(TICKS_TO_HIT)		;a983	3a 41 e5
	inc a			        ;a986	3c
	ld (TICKS_TO_HIT),a		;a987	32 413c
	pop af			        ;a98a	f1
	add a,b			        ;a98b	80
	cp (hl)			        ;a98c	be
...
la993h:
	dec (hl)			    ;a993	35
la994h:
	ld a,(TICKS_TO_HIT)		;a994	3a 41 e5
	ld b,a			        ;a997	47
	ld a,(BALL_Y_SLOPE)		;a998	3a 43 e5
	ld c,a			        ;a99b	4f
	neg		                ;a99c	ed 44
la99eh:
	add a,c			        ;a99e	81
...
la9f0h:
	ld b,a			;a9f0	47
la9f1h:
	ld (ix+SPR_PARAMS_IDX_Y),b		    ;a9f1	dd 70 00
...
laa01h:
	ld (ix+SPR_PARAMS_IDX_X),a		    ;aa01	dd 77 01
	ret			                        ;aa04	c9
